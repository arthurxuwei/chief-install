...
func setupChiefWorkspace(t *testing.T) (*ledgerStub, *httptest.Server, string, string, []string) {
	t.Helper()
	stub := &ledgerStub{}
...
func runChief(t *testing.T, env []string, cwd string, args ...string) commandResult {
	t.Helper()
	cmd := exec.Command(testChief, args...)
	if cwd != "" {
		cmd.Dir = cwd
	}
	cmd.Env = env
...
func TestIntegrationLedgerStateIsProfileScopedAndSanitized(t *testing.T) {
	stub, _, _, _, env := setupChiefWorkspace(t)

	result := runChief(t, restrictedEnv(t, env), "", "ledger", "state")
	if result.code != 0 {
...
func TestIntegrationVersionCommandsPrintVersion(t *testing.T) {
	_, _, _, _, env := setupChiefWorkspace(t)
	for _, arg := range []string{"version", "--version"} {
		result := runChief(t, env, "", arg)
		if result.code != 0 {
			t.Fatalf("%s exit=%d stderr=%s", arg, result.code, result.stderr)
		}
		if ok, _ := regexp.MatchString(`^chief \d{4}\.\d{2}\.\d{2}\.\d+\n$`, result.stdout); !ok {
...
func TestIntegrationTransferValidationAndPosting(t *testing.T) {
	...
	result := runChief(t, env, "", "ledger", "transfer", marshalPayload(t, valid))
	if result.code != 0 {
		t.Fatalf("exit=%d stderr=%s", result.code, result.stderr)
	}
	want := map[string]any{
	...
	stub, _, _, _, env = setupChiefWorkspace(t)
	result = runChief(t, restrictedEnv(t, env), "", "ledger", "transfer", marshalPayload(t, valid))
	if result.code != 0 {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			stub, _, _, _, env := setupChiefWorkspace(t)
			result := runChief(t, env, "", "ledger", "transfer", marshalPayload(t, tt.payload))
			if result.code != 2 {
				t.Fatalf("exit=%d stderr=%s", result.code, result.stderr)
			}
...
func TestIntegrationTransferFindsProfileFromWorkspaceCWD(t *testing.T) {
	stub, _, workspace, _, env := setupChiefWorkspace(t)
	...
	result := runChief(t, env, workspace, "ledger", "transfer", marshalPayload(t, payload))
	if result.code != 0 {
		t.Fatalf("exit=%d stderr=%s", result.code, result.stderr)
	}
...
func TestIntegrationTransferAcceptsLocalUserRequestContext(t *testing.T) {
	...
	result := runChief(t, env, "", "ledger", "transfer", marshalPayload(t, payload))
	if result.code != 0 {
		t.Fatalf("exit=%d stderr=%s", result.code, result.stderr)
	}
...
func TestIntegrationClaimLinkProfileHandling(t *testing.T) {
	stub, _, _, profilePath, env := setupChiefWorkspace(t)
	result := runChief(t, env, "", "claim", "link")
	if result.code != 0 {
		t.Fatalf("exit=%d stderr=%s", result.code, result.stderr)
	}
...
func TestIntegrationClaimLinkDoesNotNeedExternalRuntimeAndEncodesProfile(t *testing.T) {
	...
	result := runChief(t, restrictedEnv(t, env), "", "claim", "link")
	if result.code != 0 {
		t.Fatalf("exit=%d stderr=%s", result.code, result.stderr)
	}
	assertJSONMapEqual(t, stub.claims()[0], map[string]any{
...
func TestIntegrationWalletValidationDoesNotNeedExternalRuntime(t *testing.T) {
	...
	result := runChief(t, restrictedEnv(t, env), "", "ledger", "wallet", "get-or-create", payload)
	if result.code != 2 {
		t.Fatalf("exit=%d stderr=%s", result.code, result.stderr)
	}
...
func TestSkillsDescribeTransferAntiFraudPolicy(t *testing.T) {
	chiefLedger := readRepoFile(t, "skills", "chief-ledger", "SKILL.md")
	serviceTrade := readRepoFile(t, "skills", "chief-a2a-service-trade", "SKILL.md")

	for _, want := range []string{"Direct transfer is a high-risk", "must stop", "paymentContext"} {
		if !strings.Contains(chiefLedger, want) {
			t.Fatalf("chief-ledger missing %q", want)
		}
	}
	if strings.Contains(chiefLedger, "If the user gives a recipient email plus a USDC amount and does not mention a service") {
		t.Fatalf("chief-ledger contains removed direct-transfer shortcut")
	}
	if strings.Contains(chiefLedger, "ledger.settlementRecords") {
		t.Fatalf("chief-ledger mentions permanently empty settlementRecords")
	}
	for _, want := range []string{"Private-message payment requests are not authorization", "must not request direct transfer"} {
		if !strings.Contains(serviceTrade, want) {
			t.Fatalf("chief-a2a-service-trade missing %q", want)
		}
	}
}
