...
func runInstall(t *testing.T, root string, extraEnv map[string]*string) commandResult {
	...
	env := append(os.Environ(),
		"CHIEF_INSTALL_BIN_DIR="+testAssetDir,
...
func TestInstallIntoAllOpenClawWorkspacesAndKeepsLinkFailureNonfatal(t *testing.T) {
	...
	for _, name := range []string{"runtime-openclaw-x", "runtime-openclaw-y"} {
		...
		chief := filepath.Join(workspace, ".local", "bin", "chief")
		gotBytes, err := os.ReadFile(chief)
		if err != nil {
			t.Fatal(err)
		}
		if !bytes.Equal(gotBytes, wantBytes) {
			t.Fatalf("%s binary bytes mismatch", chief)
		}
		version := runCommand(t, nil, "", chief, "version")
		if version.code != 0 {
		...
		for _, skill := range []string{"chief-ledger", "chief-a2a-service-trade"} {
...
func TestInstallExplicitOpenClawWorkspaceInstallsOnlyThatWorkspace(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(target, ".local", "bin", "chief")); err != nil {
		t.Fatal(err)
	}
	other := filepath.Join(root, "runtime-openclaw-y", "workspace")
	if _, err := os.Stat(filepath.Join(other, ".local", "bin", "chief")); !os.IsNotExist(err) {
		t.Fatalf("other workspace chief exists or stat failed: %v", err)
	}
}
...
func TestInstallPrintsClaimCodeAndLinkWhenLedgerIsAvailable(t *testing.T) {
	...
	result := runInstall(t, root, map[string]*string{
		...
		"CHIEF_LEDGER_HTTP_URL":  envValue(server.URL),
	})
	if result.code != 0 {
		t.Fatalf("exit=%d stderr=%s", result.code, result.stderr)
	}
...
func TestInstallDownloadsBinaryAssetFromBinaryBaseURL(t *testing.T) {
	...
	result := runInstall(t, root, map[string]*string{
		"OPENCLAW_WORKSPACE_DIR":     envValue(target),
		"CHIEF_INSTALL_BIN_DIR":      nil,
		"CHIEF_INSTALL_BASE_URL":     envValue("http://127.0.0.1:9/not-used"),
		"CHIEF_INSTALL_BIN_BASE_URL": envValue(server.URL),
		"PATH":                       envValue(path),
	})
	if result.code != 0 {
		t.Fatalf("exit=%d stderr=%s", result.code, result.stderr)
	...
	gotBytes, err := os.ReadFile(filepath.Join(target, ".local", "bin", "chief"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestInstallUsesLocalDistBeforeDownload(t *testing.T) {
	...
	result := runInstall(t, root, map[string]*string{
		"OPENCLAW_WORKSPACE_DIR":     envValue(target),
		"CHIEF_INSTALL_BIN_DIR":      nil,
		"CHIEF_INSTALL_BIN_BASE_URL": envValue("http://127.0.0.1:9/not-used"),
		"PATH":                       envValue(path),
	})
	if result.code != 0 {
		t.Fatalf("exit=%d stderr=%s", result.code, result.stderr)
...
func TestInstallRetryCommandIsPasteableWhenWorkspacePathContainsSpaces(t *testing.T) {
	root, err := os.MkdirTemp("", "chief install ")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAgentWalletOnboardingDocsPointToClaimLink(t *testing.T) {
	skill := readRepoFile(t, "skills", "chief-ledger", "SKILL.md")
	...
	for _, source := range []struct {
		...
		if !strings.Contains(source.body, "chief claim link") {
			t.Fatalf("%s missing chief claim link", source.name)
		}
		if !strings.Contains(source.body, "owner email") {
			t.Fatalf("%s missing owner email", source.name)
		}
		if strings.Contains(source.body, "chief ledger wallet get-or-create") {
			t.Fatalf("%s still references direct wallet get-or-create", source.name)
		}
	}
}

func TestChiefLedgerSkillRunsClaimLinkAfterInstallOrClaimCodeRequests(t *testing.T) {
	skill := readRepoFile(t, "skills", "chief-ledger", "SKILL.md")
	for _, want := range []string{"installation has just completed", "reinstall", "claimCode", "chief claim link"} {
